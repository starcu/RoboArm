...
void SimpleSocketServer::createSocket()
{
    ...
    {
        throw std::runtime_error("SERVER_FAILED_SOCKET_CREATION");
    }
    ...
    if (bind(server_fd, (struct sockaddr *)&address, 
                                 sizeof(address))<0)
    {
        throw std::runtime_error("SERVER_INIT_BIND_FAILED_EXIT_FAILURE");
    }

    if (listen(server_fd, 3) < 0)
    {
        throw std::runtime_error("SERVER_LISTEN_ERROR_EXIT_FAILURE");
    }
}
...
void SimpleSocketServer::listenOnSocket(bool block)
{
    do // with block set to false its possible to use this function in external while loop
    {
        if ((new_socket = accept(server_fd, 
                                ...
                                (socklen_t*)&addrlen))<0)
        {
            throw std::runtime_error("SERVER_ACCEPT_INCOME_REQ_ERROR");
        }
